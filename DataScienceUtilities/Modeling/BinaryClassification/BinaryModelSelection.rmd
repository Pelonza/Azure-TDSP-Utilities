...
list.of.packages <- c('glmnet', 'yaml', 'randomForest', 'xgboost', 'lattice', 'shiny', 'gridExtra','lme4','pROC','ROCR','RODBC','rminer','doSNOW')
...
library(rminer)
library(doSNOW)
#source("https://raw.githubusercontent.com/tobigithub/R-parallel/gh-pages/R/code-setups/Install-doSNOW-parallel-DeLuxe.R")
```
...
if (runXgBoost == TRUE) {
  ...
  xgBoostsubsample <- eval(parse(text=config$xgBoostParams[8]));
  xgBoostObjective <- eval(parse(text=config$xgBoostParams[9]));
  xgBoostGrid = expand.grid(.nrounds = nrounds, .eta = eta, .max_depth = max_depth, .gamma = gamma, .colsample_bytree = colsample_bytree, .min_child_weight = min_child_weight, .subsample = xgBoostsubsample);
